...
class EndScreenUi:
    ...
    def show_end_screen(self, winner: Color | None = None):
        ...
        if winner:
            self._piece_image.texture = self._white_king_texture if winner == Color.WHITE else self._black_king_texture
            self._label = f"{"WHITE" if winner == Color.WHITE else "BLACK"} WINS"
        else:
            self._piece_image.texture = self._white_king_texture
            self._label = "STALEMATE"
