...
_FADE_SPEED = 550   # alpha units per second
_FLOAT_AMP  = 12    # pixels
_FLOAT_FREQ = 1.8   # radians per second
...
class MainMenuView(arcade.View):
    def __init__(self):
        ...
        # --- animation state ---
        self._time       = 0.0
        self._white_y    = 0.0   # current float offset for white pawn
        self._black_y    = 0.0
        self._fade_alpha = 255.0  # 255 = black overlay, 0 = fully visible
        self._fade_state = "in"   # "in" | "out" | "idle"
        self._next_widget: arcade.gui.UIWidget | None = None

        white_tex = arcade.load_texture("assets/sprites/white_pieces/pawn_white.png")
        black_tex = arcade.load_texture("assets/sprites/black_pieces/pawn_black.png")

        self.manager.add(arcade.gui.UILabel(
            "CHESS", font_size=80, font_name="ArcadeClassic", y=470, x=250
        ))

        self._white_pawn = arcade.gui.UIImage(texture=white_tex, width=108, height=144, y=450, x=130, angle=340)
        self._black_pawn = arcade.gui.UIImage(texture=black_tex, width=108, height=144, y=450, x=550, angle=20)
        self.manager.add(self._white_pawn)
        self.manager.add(self._black_pawn)
    ...
    def on_update(self, delta_time: float):
        ...
        # floating pawn animation
        new_white = math.sin(self._time * _FLOAT_FREQ) * _FLOAT_AMP
        new_black = math.sin(self._time * _FLOAT_FREQ + math.pi) * _FLOAT_AMP
        ...
        if self._fade_state == "in":
            self._fade_alpha = max(0.0, self._fade_alpha - _FADE_SPEED * delta_time)
            if self._fade_alpha == 0.0:
                self._fade_state = "idle"

        elif self._fade_state == "out":
            self._fade_alpha = min(255.0, self._fade_alpha + _FADE_SPEED * delta_time)
            if self._fade_alpha >= 255.0 and self._next_widget is not None:
                # swap widgets at peak blackness
                self.manager.remove(self._active_widget)
                self._active_widget = self._next_widget
                self._next_widget   = None
                self.manager.add(self._active_widget)
                self._fade_state = "in"
    ...
    def on_show_view(self):
        self.manager.enable()
        # reset float state so pawns don't jump on re-entry
        self._white_y    = 0.0
        self._black_y    = 0.0
