...
class PauseUi:
    ...
    def _set_up_pause_ui(self):
        ...
        self._quit_match_button = arcade.gui.UIFlatButton(
            text="QUIT THE GAME", y=350, x=40, width=285, height=55, style=BUTTTON_STYLE)

        self._pause_manager.add(self._resume_button)
        self._pause_manager.add(self._quit_match_button)

    def _snapshot_button_styles(self):
        """Capture each button's original RGB values once so animation never drifts."""
        self._buttons = (self._resume_button, self._quit_match_button)
        self._btn_base = []
    ...
    def set_up_ui_buttons(self, resume_func: callable, quit_func: callable):
        self._resume_button.on_click = lambda _: resume_func()
        self._quit_match_button.on_click = lambda _: quit_func()
